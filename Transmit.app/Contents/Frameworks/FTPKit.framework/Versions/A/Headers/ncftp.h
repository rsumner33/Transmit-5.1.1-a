/* ncftp.h
 *
 * Copyright (c) 1996-2009 Mike Gleason, NcFTP Software.
 * All rights reserved.
 *
 */
...
#ifndef _IPv6_
#define _IPv6_ 1 /* enables IPv6 support -- SPF (will) */
#endif

#define kLibraryVersion "@(#) LibNcFTP 3.2.3 (July 23, 2009)"
...
/* IPv6 support -- SPF (ned) */
typedef union FTPSockAddr {
	struct sockaddr_in in;
	struct sockaddr_in6 in6;
} FTPSockAddr;

#define FTPSockAddrLength(sa__) (sa__).in.sin_len
#define FTPSockAddrFamily(sa__) (sa__).in.sin_family
/* IPv6 support */
...
typedef struct OpaqueTLSContext* TLSContextRef;
typedef TLSContextRef (*TLSOpenProc)(int sockfd, FTPCIPtr cip, int* result);
typedef void (*TLSCloseProc)(FTPCIPtr cip, TLSContextRef TLS);
typedef void (*TLSShutdownProc)(FTPCIPtr cip);
typedef int (*TLSReadProc)(TLSContextRef TLS, char *const buf0, size_t size, int tlen, int retry);
typedef int (*TLSWriteProc)(TLSContextRef TLS, const char *const buf0, size_t size, int tlen, int swopts);
typedef int (*TLSReadlineProc)(TLSContextRef TLS, SReadlineInfo *srl, char *const linebuf, size_t linebufsize);

/* Added -- SPF (ian) */
typedef void (*FTPDidFallBackToSendPortModeProc)(const FTPCIPtr);
typedef void (*FTPTransferProc)(const FTPCIPtr);
/* END Added -- SPF (ian) */

typedef struct FTPConnectionInfo {
	char magic[16];				/* Don't modify this field. */
	
	char host[256];				/* REQUIRED input parameter. */ /* increased to 256 -- SPF */
	char user[256];				/* OPTIONAL input parameter. */ /* increased to 256 -- SPF */
	char pass[256];				/* OPTIONAL input parameter. */
	char acct[256];				/* OPTIONAL input parameter. */ /* increased to 256 -- SPF */
	...
	char firewallHost[256];			/* OPTIONAL input parameter. */ /* increased to 256 -- SPF */
	char firewallUser[256];			/* OPTIONAL input parameter. */ /* increased to 256 -- SPF */
	char firewallPass[256];			/* OPTIONAL input parameter. */ /* increased to 256 -- SPF */
	...
	char actualHost[256];			/* Do not modify this field. */ /* increased to 256 -- SPF */
	char ip[54];				/* Do not modify this field. */ /* increased to 54 (INET6_ADDRSTRLEN + 8) -- SPF (ned) */
	...
	/* IPv6 support -- SPF (ned) */
	int hasEPRT;				/* Do not modify this field. */
	int hasEPSV;				/* Do not modify this field. */
	/* end IPv6 */
...
#if !_IPv6_ /* IPv6 support -- SPF (ned) */
...
#else
	FTPSockAddr servCtlAddr;		/* Do not modify this field. */
	FTPSockAddr servDataAddr;	/* Do not modify this field. */
	FTPSockAddr ourCtlAddr;		/* Do not modify this field. */
	FTPSockAddr ourDataAddr;		/* Do not modify this field. */
#endif /* end IPv6 */
	...
	/* TLS support -- SPF (dave/ned/will) */
	int useSSL;
	int useTLS;
	char TLSmode; /* data channel protection level: 'C' for clear, 'P' for private */
	int useCCC;
	int reuseTLSSession;
    void *reusedTLSSession;
    void *ctx;
	TLSOpenProc openTLSProc;
	TLSCloseProc closeTLSProc;
	TLSShutdownProc shutdownTLSProc;
	TLSReadProc readTLSProc;
	TLSWriteProc writeTLSProc;
	TLSReadlineProc readlineTLSProc;
	TLSContextRef controlTLS;
	TLSContextRef dataTLS;
	
    int useOnlyTLSv1;
	int useTLSv12;
	int (*cert_verify_cb)(void* cert, void* arg); /* will be called with pUser as arg, nonzero return indicates success */
	/* end TLS */
	
	/* PRET support -- SPF (ned) */
	int usePRET;
	/* end PRET */
	
	/* UTF8 support -- SPF (ned) */
	int useUTF8;
	/* end UTF8 */

	/* explicit IP connection -- SPF (will) */
	int ipFamily;
	
	/* Added -- SPF (ian) */
	FTPDidFallBackToSendPortModeProc didFallBackToSendPortModeProc;
	FTPTransferProc	uploadProc;
	FTPTransferProc downloadProc;
	/* END Added -- SPF (ian) */

	/* Disables date maintainance after transfer - SPF (wade) */
	int preserveModificationDates;
...
#if USE_SIO
	char srlBuf[10240];//SPF (will) needed for very large server banners - 768
...
/* SPF - add'l firewall types */
#define kFirewallSOCKS4					10
#define kFirewallSOCKS4a				11
#define kFirewallSOCKS5					12
#define kFirewallUserAtFwuAtSite		13
#define kFirewallLastType				kFirewallUserAtFwuAtSite
/* SPF END */
...
#else
int FTPSendPort(const FTPCIPtr cip, FTPSockAddr *saddr);
int FTPSendPassive(const FTPCIPtr cip, FTPSockAddr *saddr, int *weird);
#endif /* end IPv6 */
...
/* SOCKS proxy support --SPF (will) */
int HandleSOCKS4Connection(volatile int socketfd, const FTPCIPtr cip, char *host, unsigned int port, struct sockaddr_in *pHostInfo);
int HandleSOCKS4aConnection(volatile int socketfd, const FTPCIPtr cip, char *host, unsigned int port, struct sockaddr_in *pHostInfo);
int HandleSOCKS5Connection(volatile int socketfd, const FTPCIPtr cip, char *host, unsigned int port, FTPSockAddr *pHostInfo);
